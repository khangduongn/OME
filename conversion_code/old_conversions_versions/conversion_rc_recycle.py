...
if __name__ == '__main__':
    ...
    fileHandler = logging.FileHandler(os.path.join(args.input_dir, 'flagged_images.csv'))
    ...
    for tiles_path in image_paths:
        ...
        try:
            ...
            if type(results) is tuple:
                ...
                # Assuming tiles_path stores the path above "TOBEMOVED"
                # Find the directory to be moved
                #tobemoved_path = os.path.dirname(tiles_path.rstrip(os.sep))

                # Create the corresponding directory structure in the recycle bin
                #relative_path = os.path.relpath(tobemoved_path, args.input_dir)
                #recycle_bin_dir = os.path.join(args.recycle_bin, relative_path)
                #os.makedirs(recycle_bin_dir, exist_ok=True)

                # Move the tobMoved_path to the recycling_path within the input directory in the recycle bin
                #recycling_path = os.path.join(recycle_bin_dir, os.path.basename(tobemoved_path))
                #shutil.move(tobemoved_path, recycling_path)
                #logging.info(f"Moving {tobemoved_path} to {recycling_path}")
