'''
Author: 
    Khang Duong, Rohan Chandraghatgi
...
Future Improvements:
    ...
    TODO: IMPORTANT! Add error handling for drift
...
parser.add_argument('-r','--recycle-bin', type=str, metavar='', required=True, help='Path of the directory that input dir will be moved to upon successful conversion')

args = parser.parse_args()
"""

def insert_tile(img_stitched, tiles_path, tileNumber, tilesize_x, tilesize_y, R, Rmax, C, Cmax, Z, ext, overlap, x_pixel_drift, y_pixel_drift ):
    
    #TODO: ERROR HANDLE. Need Testing
    if '.tif' in ext:
        if os.path.isfile(os.path.join(tiles_path, f'Tile{str(tileNumber).zfill(6)}' + ext)):
            tile = tifffile.imread(os.path.join(tiles_path, f'Tile{str(tileNumber).zfill(6)}' + ext))
        else:
            raise Exception(f"{tiles_path},Tiles are missing. Tiles could not be read or inserted into stitched image.")
    else:
        raise Exception("Tiles are not of the .tif format. Please only use uncompressed tiles.")
        #tile = cv2.imread(os.path.join(tiles_path, f'Tile{str(tileNumber).zfill(6)}' + ext), cv2.IMREAD_UNCHANGED)

    
    yRange = slice(R * (tilesize_y - overlap) + y_pixel_drift, (R + 1) * (tilesize_y - overlap) + overlap + y_pixel_drift) 
    tile = tile[0:(tilesize_y), :]        

    #if at even row (0, 2, 4 ...)
    if (R % 2 == 0):

        #if at last column
        if (C == Cmax):
            xRange = slice(C * (tilesize_x - overlap) + x_pixel_drift, (C + 1) * (tilesize_x - overlap) + overlap + x_pixel_drift)
            tile = tile[:, 0:(tilesize_x)]
        else:
            xRange = slice(C * (tilesize_x - overlap) + x_pixel_drift, (C + 1) * (tilesize_x - overlap) + x_pixel_drift)
        ...
        #if at first column
        if (C == 0):
            xRange = slice(x_pixel_drift, tilesize_x + x_pixel_drift)            
        else:
            xRange = slice((C - 1) * (tilesize_x - overlap) + tilesize_x + x_pixel_drift, (C) * (tilesize_x - overlap) + tilesize_x + x_pixel_drift)
            tile = tile[:, (overlap):(tilesize_x)]

    # If a border tile:
    if (R == 0) or (R == Rmax) or (C == 0)  or (C == Cmax):
        pixel_val = 176 #stats.mode(tile, axis = None)[0]
        if (R == 0):
            img_stitched[Z, slice(0, y_pixel_drift), xRange] = pixel_val
        elif (R == Rmax):
            img_stitched[Z, slice(yRange.stop, img_stitched.shape[1]), xRange] = pixel_val
        if (C == 0):
            img_stitched[Z, yRange, slice(0, x_pixel_drift)] = pixel_val
        elif (C == Cmax):
            img_stitched[Z, yRange, slice(xRange.stop, img_stitched.shape[2])] = pixel_val

    img_stitched[Z, yRange, xRange]  = tile


def stitch_tiles(tiles_path, staging_path):
    '''
    Description:
    ...
    # Check file extension to make sure tiles are raw .tif files
    if '.tif' not in ext:
        XYZdata = None # stops code from executing if tiles are not of the .tif type
        raise Exception("Tiles are not of the .tif format. Please only use uncompressed tiles.")
    ...
    if nx != 1 and ny != 1:
        ...
        #determines x-drift by # of pixels, making row with most number of unique columns reference row

        R_ref = XYZdata[R_col].mode()[0]
        x_init_col = XYZdata[X_col].loc[(XYZdata[R_col] == R_ref) & (XYZdata[Z_col] == Z_min)].values

        #if odd number, flips reference array so that indexes line up
        if R_ref % 2 == 1:
            x_init_col = np.flip(x_init_col)

        x_drift = XYZdata[X_col].values - x_init_col[XYZdata[C_col].values]
        x_pixel_drift = np.rint(x_drift * resolution).astype(int)
        ...
        #determines y-drift by # of pixels, making column with most number of unique rows reference column
        C_ref = XYZdata[C_col].mode()[0]
        y_init_col = XYZdata[Y_col].loc[(XYZdata[C_col] == C_ref) & (XYZdata[Z_col] == Z_min)].values
        y_drift = XYZdata[Y_col].values - y_init_col[XYZdata[R_col].values]
        y_pixel_drift = np.rint(y_drift * resolution).astype(int)
    ...
    stitched_x, stitched_y, stitched_z = (tile_xsize - overlap)*nx + overlap + x_pixel_drift.ptp(), (tile_ysize-overlap)*ny + overlap + y_pixel_drift.ptp(), nz
    
    #adjust pixel drifts to shift minimum to 0
    x_pixel_drift = x_pixel_drift - x_pixel_drift.min()    
    y_pixel_drift = y_pixel_drift - y_pixel_drift.min()
    ...
    filename = os.path.join(staging_path, f'{uuid.uuid4().hex}_{os.path.basename(tiles_path)}.dat')


    #store image data to the memory mapped file
    img_stitched = np.memmap(filename, dtype='uint8', mode='w+', shape=img_shape)
    #img_stitched[:] = 176 #avg color (hardcoded)
    ...
    #with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        # [executor.submit(insert_tile, img_stitched, tiles_path, XYZdata.iloc[i, 0], tile_xsize, tile_ysize, XYZdata.iloc[i, 4], R_max, XYZdata.iloc[i, 5], C_max,  Zs_list.index(XYZdata.iloc[i, 7]), ext, overlap, x_pixel_drift[i], y_pixel_drift[i]) for i in range(len(XYZdata))]
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        for i in range(len(XYZdata)):
            future = executor.submit(
                ...
                XYZdata.iloc[i, 5], 
                C_max,  
                Zs_list.index(XYZdata.iloc[i, 7]), 
                ext, 
                overlap, 
                x_pixel_drift[i], 
                y_pixel_drift[i]
    ...
    for future in futures:
        try:
            future.result()  # This will raise any exception encountered within the thread
        except Exception as e:
            error_message = str(e)
            if error_message not in unique_errors:
                logging.error(f"Error in processing tile: {error_message}")
                unique_errors.add(error_message)
            successful = False

    if successful:
        img_stitched.flush()  # Only flush if stitching was successful
        del img_stitched  # Remove from memory if successful
        ...
        logging.info("Stitching was unsuccessful; no file will be created.")
...
def convert_slide(args):
    ...
    if not logger.handlers:
        ...
        fileHandler = logging.FileHandler(os.path.join(args.failure_dir, 'flagged_images.csv'))
    ...
    # gets the path of the tile images
    image_paths = [os.path.dirname(img_dir) for img_dir in XYZ_path]

    # if there are no paths stored in the list image_paths
    ...
    for tiles_path in image_paths:
        ...
        try:
            ...
            results = stitch_tiles(tiles_path, args.staging_dir)
                ...
                if args.pyramid_resolutions == 0:
                    ...
                    with tifffile.TiffWriter(outputImagePath, bigtiff=args.bigtiff) as tif:
                        options = dict(tile=(tileSizeX, tileSizeY), compression=compression_parameter, 
                            metadata={'axes': 'ZYX', 'PhysicalSizeX': pixSizeX, 'PhysicalSizeXUnit': "µm", 'PhysicalSizeY': pixSizeY, 'PhysicalSizeYUnit':"µm"}, maxworkers=16)
